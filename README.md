...
- **Progress, ETA, and Cancel**, per-step progress while scanning, with a Cancel button on screen for every phase.
- **Photo info panel**, dimensions, file size, and created/modified dates (to the second) for any result, plus which shared album an iCloud photo came from.
- **Honest about iCloud-only photos**, photos whose originals live only in iCloud are recognized as such (not reported as failures), and are skipped for hashing without being re-attempted on every scan.
